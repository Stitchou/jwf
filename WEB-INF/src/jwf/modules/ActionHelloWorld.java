...
public class ActionHelloWorld implements IAction {
	...
	public void proceed(IContext context) {
		try {
			...
			context._getResponse().getOutputStream().println("<h1>Hello world !!!</h1>");
			context._getResponse().getOutputStream().println("</head>");
			context._getResponse().getOutputStream().println("<body>");
			context._getResponse().getOutputStream().println("<br> <br>");
			context._getResponse().getOutputStream().println("<A HREF=\"http://localhost:8080/jwf/user/login\">Log in</A>");
			context._getResponse().getOutputStream().println("<br> <br>");
			context._getResponse().getOutputStream().println("<A HREF=\"http://localhost:8080/jwf/create\">Create</A>");	
			context._getResponse().getOutputStream().println("</body>");
